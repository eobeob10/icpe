...
# -----------------------------
# 1) Time-based split (train: oldest 80%, test: newest 20%)
# -----------------------------
time_col = None
...
print("Train:", t_sorted.iloc[0], "to", t_sorted.iloc[cut - 1], "n=", len(train_idx))
print("Test :", t_sorted.iloc[cut], "to", t_sorted.iloc[-1], "n=", len(test_idx))
...
train_ds, test_ds = make_hf_datasets(TEXT_COL)
...
assert "train_idx" in globals() and "test_idx" in globals(), "Run Cell 6 first (train_idx/test_idx)."
...
train_ds, test_ds = make_hf_datasets(CONFIG["text_col"])
...
training_args = TrainingArguments(
    output_dir=f"./bert_runs/{CONFIG['model_name'].replace('/', '-')}-v2",
    eval_strategy="epoch",
    save_strategy="epoch",
...
trainer = WeightedTrainer(
    ...
    eval_dataset=test_tok,  # Using test_tok as validation for now
    tokenizer=tokenizer,
...
# 1. Create a data collator that will pad your batches dynamically
data_collator = DataCollatorWithPadding(tokenizer=tokenizer)

# 2. Update your Trainer to use this collator
eval_trainer = Trainer(
    model=model,
    eval_dataset=test_tok,  # Your test dataset
    compute_metrics=compute_metrics,
    tokenizer=tokenizer,  # <--- Pass the tokenizer here
    data_collator=data_collator  # <--- OR explicitly pass the collator here
)
